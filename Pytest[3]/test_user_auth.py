...
class TestUserAuth:
    ...
    def test_negative_auth_check(self, condition):
        ...
        if condition == 'no_cookie':
            response = requests.get(url='https://playground.learnqa.ru/api/user/auth', headers={'x-csrf-token': token})
        else:
            response = requests.get(url='https://playground.learnqa.ru/api/user/auth', cookies={'auth_sid': auth_sid})

        assert 'user_id' in response_auth.json(), "There is no user id in the first response"
        user_id_from_check_method = response.json()['user_id']
        assert user_id_from_check_method == 0, f"User is authorize with condition {condition}"
